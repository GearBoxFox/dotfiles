...
local function next()
	if entryindex ~= #filtered then
		...
		entries:get_widgets_at(entryindex + 1, 1)[1].bg = x.color10
...
local function filter(cmd)
	...
	for i, entry in ipairs(filtered) do
							...
							{
								{
									-- text = entry.name,
									markup = '<span color="' ..
											x.color12 .. '" font="Ubuntu Nerd Font 15 bold">' .. entry.name .. '</span>',
								...
								{
									markup = '<span color="' ..
											x.color13 .. '" font="Ubuntu Nerd Font 13">' .. description .. '</span>',
									widget = wibox.widget.textbox,
									-- forced_height = dpi(50)
								},
			...
			buttons = {
				awful.button({}, 1, function()
					if entryindex == i then
						...
						entries:get_widgets_at(entryindex, 1)[1].bg = nil
						entryindex = i
						entries:get_widgets_at(entryindex, 1)[1].bg = beautiful.background_lighter
					end
				end),
		...
		if i == entryindex then
			widget.bg = beautiful.background_lighter
		end
	end
...
local function open()
	...
	awful.prompt.run {
		-- prompt = "Search: ",
		prompt = '<span color="' ..
				x.color13 .. '" font="Ubuntu Nerd Font Bold 16">' .. '  : ' .. '</span>',
		textbox = prompt,
